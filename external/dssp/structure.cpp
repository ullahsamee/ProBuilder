...
void MProtein::ReadCoords(std::vector<std::vector<std::vector<double>>> coords,bool cAlphaOnly){
  std::vector<MAtom> atoms;
  std::vector<std::string> atom_map={"N","CA","C","O"};
  for(int i =0,atom_index=0;i<int(coords.size());i++){
    for(int j =0;j<4;j++){
      atom_index++;
      MAtom atom = {};
      atom.mSerial = uint32(atom_index);
      //  13 - 16  Atom name Atom name.
      atom.mName = atom_map[j];
      //  17    Character altLoc Alternate location indicator.
      atom.mAltLoc = ' ';
      //  18 - 20  Residue name resName Residue name.
      atom.mResName = "ASP";
      ...
      //  23 - 26  Integer resSeq Residue sequence number.
      atom.mResSeq = int64(i+1);
      //  27    AChar iCode Code for insertion of residues.
      atom.mICode = ' ';

      //  31 - 38  Real(8.3) x Orthogonal coordinates for X in Angstroms.
      atom.mLoc.mX = double(coords[i][j][0]);
      //  39 - 46  Real(8.3) y Orthogonal coordinates for Y in Angstroms.
      atom.mLoc.mY = double(coords[i][j][1]);
      //  47 - 54  Real(8.3) z Orthogonal coordinates for Z in Angstroms.
      atom.mLoc.mZ = double(coords[i][j][2]);
