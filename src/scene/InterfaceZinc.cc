...
InterfaceZinc::InterfaceZinc() : InterfaceMetal() {}
InterfaceZinc::~InterfaceZinc() {}

void InterfaceZinc::load_chain1_metal_configs(std::string json_fname)
{
...
void InterfaceZinc::load_chain2_metal_configs(std::string json_fname)
{
    ...
    for(Size idx=0; idx<_num_metals_chain2; ++idx) {
        ...
        Mat m = AngleAxis( 3.14159265359/2, x_axis ) * AngleAxis( 3.14159265359, z_axis ) * tempx.rotation();
        for(Size i = 0; i < 9; ++i) tempy.data()[i] = m.data()[i];

        Mat n = AngleAxis( -3.14159265359/2, x_axis ) * AngleAxis( 3.14159265359, z_axis ) * tempx.rotation();
        for(Size i = 0; i < 9; ++i) tempz.data()[i] = n.data()[i];
        //y.rotation() = AngleAxis( 3.14159265359, v ) * x.rotation();
        tempy.translation() = tempx.translation();
        tempz.translation() = tempx.translation();

        _relative_xforms_chain2.push_back(tempy);
        _relative_xforms_chain2_flipped.push_back(tempz);
    }
    _current_xforms_chain2.resize(_num_metals_chain2);
    _current_xforms_chain2_snapshot.resize(_num_metals_chain2);
    _current_xforms_chain2_flipped.resize(_num_metals_chain2);
    _current_xforms_chain2_flipped_snapshot.resize(_num_metals_chain2);
}

void InterfaceZinc::compute_pairwise_distance()
{
            ...
            for(Size jdx=0; jdx<_num_metals_chain2; ++jdx) {
                if(_inter_chain_cluster_match[idx*_num_metals_chain2+jdx]) {
                    ...
                    if( d2 > 64 ) {
                        ...
                        Real dist1 = xform_magnitude(y*_current_xforms_chain2[jdx]);
                        Real dist2 = xform_magnitude(y*_current_xforms_chain2_flipped[jdx]);
